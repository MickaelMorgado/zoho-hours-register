  ...
  getTasksManager() {
    return {
      ...
      fetchTasksFromProjects: async (activeProjectIds) => {
        ...
        try {
          // Get credentials for building task URLs
          ...
          // Fetch tasks from all active projects using zohoFetch (auto-refreshes tokens)
          const allTasks = [];
          for (const projectId of activeProjectIds) {
            try {
              const response = await zohoFetch(`/api/zoho/projects/${projectId}/tasks`);
              ...
              if (response.ok) {
                ...
                // Look up project name from localStorage when API doesn't provide it
